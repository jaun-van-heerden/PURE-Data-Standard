...
| `ext:path_length_mi` | nws-effingham | path length as stated |
| `ext:path_width_max_yd` | nws-effingham | maximum path width as stated |
| `ext:end_time` | nws-effingham | event end instant, event-local offset |
| `ext:end_geo` | nws-effingham | lift point as a `geo:` geohash |
...
| [`fixtures/disaster.tornado/nws-effingham/`](../../fixtures/disaster.tornado/nws-effingham/) | **First real source**: the NWS Lincoln IL damage survey (public domain) for the June 17, 2026 Effingham EF3. First-hand basis, stated zero (`deaths: 0` is a claim), five `ext:` fields, event-local date in the key (`2026-06-17`, though 7:56 PM CDT is June 18 UTC). Encoded via `tools/ingest.py`. |
