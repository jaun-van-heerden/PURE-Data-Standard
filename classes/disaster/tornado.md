...
Place richness follows the same pattern as everything else: `where` stays a single locus ID (it feeds the event key), and stated place facets — name, exact coordinates, venue — are sibling `location.*` claims defined in [`classes/common.md`](../common.md), each with its own quote. Three precision tiers, each fit for purpose: the key cell joins (~geohash-4), the `where` ID locates (geohash-6), `location.lat/lon` measure (as stated).
